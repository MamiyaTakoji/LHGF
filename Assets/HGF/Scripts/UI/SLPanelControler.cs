...
/*            string saveId = (PageNum * 4 + int.Parse(Go.name)).ToString();
            ...
            if (saveDatas.ContainsKey(saveId)){
                Go.GetComponent<LoaderControler>().SetSaveContent(saveDatas[saveId]);
            }
            else
            {
    ...
    public void GetCurrentData()
    {
        ...
        foreach (var Key in SaveDatas.Keys)
        {
            ...
            {
                NewestData = Data;
                newestSaveData = SaveDatas[Key];
                newestSaveDataId = Key;
            }
        }
        //CurrntPageId = NewestDataPage;
        //SetLoaderContent(NewestDataPage);
